...
def _get_all_usage_data(period_start: datetime, period_end: datetime) -> dict[str, Any]:
    ...
    return {
        ...
        "teams_with_ff_count": list(
            FeatureFlag.objects_including_soft_deleted.values("team_id").annotate(total=Count("id")).order_by("team_id")
        ),
        "teams_with_ff_active_count": list(
            FeatureFlag.objects_including_soft_deleted.filter(active=True)
            .values("team_id")
            .annotate(total=Count("id"))
            .order_by("team_id")
        ),
