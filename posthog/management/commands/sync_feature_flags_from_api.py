...
def sync_feature_flags_from_api(
    ...
    for project in Project.objects.all():
        ...
        existing_flags = FeatureFlag.objects_including_soft_deleted.filter(team__project_id=project.id).values_list(
            "key", flat=True
        )
        deleted_flags = FeatureFlag.objects_including_soft_deleted.filter(
            team__project_id=project.id, deleted=True
