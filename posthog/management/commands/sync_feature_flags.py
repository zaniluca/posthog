...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for team in Team.objects.all():
            existing_flags = FeatureFlag.objects_including_soft_deleted.filter(team=team).values_list("key", flat=True)
            deleted_flags = FeatureFlag.objects_including_soft_deleted.filter(team=team, deleted=True).values_list(
                "key", flat=True
            )
            for flag in flags.keys():
                ...
                if flag in deleted_flags:
                    ff = FeatureFlag.objects_including_soft_deleted.filter(team=team, key=flag)[0]
                    ff.deleted = False
